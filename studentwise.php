		...
		session_start();
			...
			while($rec = mysqli_fetch_array($result))
			{
				...
				<tr>
				  ...
				  <td width="160" bgcolor="#000000;"><span class="style2">Percentage</span></td>';
				...
				<tr>
				  ...
				  $rno=$rec["rollno"];
				  $sql = "select *from `attendance` where `member_id` = ".$rec["rollno"];
				 ...
				 $result1 = mysqli_query($con,$sql)or die("select error error");
				  ...
				  $sql = "select count(distinct date) from `attendance` WHERE  attendance= $n and  member_id= $rno"  ;
				 $result1 = mysqli_query($con,$sql)or die("select error error11");
					while($rec2 = mysqli_fetch_array($result1))
					{
					$pre= $rec2["count(distinct date)"];					
					...
					echo'<td> '.$fp.'% </td>';
						 ...
						 echo '<td>  '.$fp.'% : Attendance is short  </td>';
					}}
				   }
					/*$result1 = mysqli_query($con,$query1)or die("select error error");
					while($rec1 = mysqli_fetch_array($result1))
